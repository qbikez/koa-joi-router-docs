  ...
  if(noPathParamsExist && noPathValidatorExists && noParamsValidatorExists){
    ...
    if (pathCaptures) {
      each(pathCaptures, function(pathParameter){
        ...
        const existingIndex = routeDesc.parameters.findIndex(p => p.name === captured.name);
        if (existingIndex >= 0) {
          routeDesc.parameters[existingIndex] = { ...captured, ...routeDesc.parameters[existingIndex] }
        } else {
          routeDesc.parameters.push(captured);
        }
      });
    }
  }
